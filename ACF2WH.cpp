...
IMPLEMENT_DYNCREATE(CACF2WH, CACFWH)

CACF2WH::CACF2WH()
{
	INT i = 0;
	m_lStnErrorCode		= IDS_ACF2WH_NOT_SELECTED_ERR;
	m_szPRTaskStnName	= GetStation().Trim('_');
	m_lPRStnErrorCode	= IDS_ACF2WH_PR_MOD_NOT_SELECTED;
	m_lPRModSelectMode	= GLASS_MOD;

	m_lAddJobPR_GRAB	= ACF2WH_PR_GRAB;
	m_lAddJobPR_PROCESS = ACF2WH_PR_PROCESS;
	// Motor Related
	m_stMotorY.InitMotorInfo("ACF2WH_Y", &m_bModSelected);
	m_stMotorT.InitMotorInfo("ACF2WH_T", &m_bModSelected);
	//m_stMotorZ[WHZ_1].InitMotorInfo("ACF2WH_Z", &m_bModSelected);
#ifdef SRCH_EMBEDDED_SENSOR //20170810
	m_stMotorT.SetMtrSnrPortName(CString("ACF2WH_T_MTRSNR_PORT"), TRUE, TRUE); //channel has embeded limit snr and indexer inside the encoder plug. An external limit sensor as home sensor.
#endif
	...
	// Si Related
	m_stYHomeSnr	= CSiInfo("ACF2WHYHomeSnr",					"IOMODULE_IN_PORT_2", BIT_8, ACTIVE_HIGH);
	m_mySiList.Add((CSiInfo*)&m_stYHomeSnr);
	m_stTHomeSnr	= CSiInfo("ACF2WHTHomeSnr",					"IOMODULE_IN_PORT_2", BIT_9, ACTIVE_HIGH);
	m_mySiList.Add((CSiInfo*)&m_stTHomeSnr);
	m_stAnvil1VacSnr	= CSiInfo("ACF2WHAnvllLVacSnr",		"IOMODULE_IN_PORT_2", BIT_10, ACTIVE_LOW);
	m_mySiList.Add((CSiInfo*)&m_stAnvil1VacSnr);
	m_stAnvil2VacSnr	= CSiInfo("ACF2WHAnvllRVacSnr",		"IOMODULE_IN_PORT_2", BIT_11, ACTIVE_LOW);
	m_mySiList.Add((CSiInfo*)&m_stAnvil2VacSnr);
	m_stGlass1VacSnr	= CSiInfo("ACF2WHVacSnr",				"IOMODULE_IN_PORT_2", BIT_12, ACTIVE_LOW);
	m_mySiList.Add((CSiInfo*)&m_stGlass1VacSnr);


	// So Related
	m_stAnvilWBSol		= CSoInfo("ACF2WHAnvllLWBSol",		"IOMODULE_OUT_PORT_1", BIT_8, ACF1WH_SOL, ACTIVE_HIGH);
	m_mySoList.Add((CSoInfo*)&m_stAnvilWBSol);
	m_stAnvil1VacSol		= CSoInfo("ACF2WHAnvllLVacSol",		"IOMODULE_OUT_PORT_1", BIT_9, ACF1WH_SOL, ACTIVE_LOW);
	m_mySoList.Add((CSoInfo*)&m_stAnvil1VacSol);
	m_stAnvil2WeakBlowSol		= CSoInfo("ACF2WHAnvllRWBSol",		"IOMODULE_OUT_PORT_1", BIT_10, ACF1WH_SOL, ACTIVE_HIGH);
	m_mySoList.Add((CSoInfo*)&m_stAnvil2WeakBlowSol);
	m_stAnvil2VacSol		= CSoInfo("ACF2WHAnvllRVacSol",		"IOMODULE_OUT_PORT_1", BIT_11, ACF1WH_SOL, ACTIVE_LOW);
	m_mySoList.Add((CSoInfo*)&m_stAnvil2VacSol);
	m_stWeakBlowSol				= CSoInfo("ACF2WHAnvllTopWBSol",	"IOMODULE_OUT_PORT_1", BIT_12, ACF1WH_SOL, ACTIVE_HIGH);
	m_mySoList.Add((CSoInfo*)&m_stWeakBlowSol);
	m_stGlass1VacSol			= CSoInfo("ACF2WHVacSol",			"IOMODULE_OUT_PORT_1", BIT_13, ACF1WH_SOL, ACTIVE_LOW);
...
//Update Functions
/////////////////////////////////////////////////////////////////
VOID CACF2WH::RegisterVariables()
{
	try
	{
		// -------------------------------------------------------
		...
		// -------------------------------------------------------
		RegVariable(_T("HMI_bACF2WHYOn"), &m_stMotorY.bHMI_On);
		RegVariable(_T("HMI_bACF2WHTOn"), &m_stMotorT.bHMI_On);
		//RegVariable(_T("HMI_bACF2WHZOn"), &m_stMotorZ[WHZ_1].bHMI_On);
		
		RegVariable(_T("HMI_lACF2WHYEncoder"), &m_stMotorY.lHMI_CurrPosn);
		RegVariable(_T("HMI_lACF2WHTEncoder"), &m_stMotorT.lHMI_CurrPosn);
		//RegVariable(_T("HMI_lACF2WHZEncoder"), &m_stMotorZ[WHZ_1].lHMI_CurrPosn);
		...
		RegVariable(_T("HMI_bACF2WHGYLmtSnr"), &m_stYLmtSnr.bHMI_Status);
		RegVariable(_T("HMI_bACF2WHTHomeSnr"), &m_stTHomeSnr.bHMI_Status);

		RegVariable(_T("HMI_bACF2WHG1VacSnr"), &m_stGlass1VacSnr.bHMI_Status);
		RegVariable(_T("HMI_bACF2WHAnvilLVacSnr"), &m_stAnvil1VacSnr.bHMI_Status);
		RegVariable(_T("HMI_bACF2WHAnvilRVacSnr"), &m_stAnvil2VacSnr.bHMI_Status);

		RegVariable(_T("HMI_bACF2WHWeakBlowSol"), &m_stWeakBlowSol.bHMI_Status);
		RegVariable(_T("HMI_bACF2WHG1VacSol"), &m_stGlass1VacSol.bHMI_Status);
		RegVariable(_T("HMI_bACF2WHAnvilWBSol"), &m_stAnvilWBSol.bHMI_Status);
		RegVariable(_T("HMI_bACF2WHAnvilVacSol"), &m_stAnvil1VacSol.bHMI_Status);
		RegVariable(_T("HMI_bACF2WHAnvil2WBSol"), &m_stAnvil2WeakBlowSol.bHMI_Status);
		RegVariable(_T("HMI_bACF2WHAnvil2VacSol"), &m_stAnvil2VacSol.bHMI_Status);
